...
void UNivelbasico1::BuilderEnemigos(UWorld* World)
{
    if (!World) return;
    ...
    if (FabricaAgresor)
    {
        ...
        for (int i = 0; i < 2; i++) // cantidad de explosivos
        {
            AEnemigo* EnemigoExplosivo = FabricaAgresor->CrearEnemigo("Explosivo");
            if (EnemigoExplosivo)
            {
                FVector Pos = PosicionAleatoria(MinX, MaxX, MinY, MaxY, Altura);
                EnemigoExplosivo->SetActorLocation(Pos);
            }
        }
    }
...
void UNivelbasico1::BuilderBonus(UWorld* World)
{
    if (!World) return;
